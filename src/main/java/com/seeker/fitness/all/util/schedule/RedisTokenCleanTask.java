...
public class RedisTokenCleanTask extends TimerTask {
    ...
    public void run() {
        ...
        try {
            log.debug(">>>-----------------redis清理token计划任务开始("+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+")-----------------<<<");
            //拿到用户登陆记录集合信息
            ...
            log.debug(">>>-----------------redis清理token计划任务结束("+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+")-----------------<<<");
        }catch (Exception e){
            e.printStackTrace();
        }finally {
            jedis.close();
